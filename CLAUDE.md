...
warnings - \[ \] `pytest tests/test_pyutschooldata.py` — all tests
pass - \[ \]
...
presence in the raw data. Utah USBE data includes demographic, gender,
and special population columns.
...
[`enr_grade_aggs()`](https://almartin82.github.io/utschooldata/reference/enr_grade_aggs.md):
`K8`, `HS`, `K12`

**Note:** Raw data uses various grade naming patterns (e.g.,
`Preschool`, `Pre_K`, `PreK`, `Kindergarten`, `Grade_1`, `G1`). These
are all normalized to `PK`, `K`, `01`-`12` in tidy output. Always filter
...
Charter status via `charter_flag` column.
