...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ut.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyutschooldata as ut
        assert hasattr(ut, 'fetch_enr')
        assert hasattr(ut, 'fetch_enr_multi')
        assert hasattr(ut, 'tidy_enr')
        assert hasattr(ut, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyutschooldata as ut
        assert hasattr(ut, '__version__')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ut.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = ut.fetch_enr(years['max_year'])
        # Total enrollment in Utah varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Utah should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ut.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ut.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyutschooldata as ut
        years = get_test_years()
        df_single = ut.fetch_enr(years['max_year'])
        df_multi = ut.fetch_enr_multi([years['max_year']])
