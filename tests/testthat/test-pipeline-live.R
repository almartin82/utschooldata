# ==============================================================================
# LIVE Pipeline Tests for utschooldata
# ==============================================================================
#
...
test_that("Utah DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Utah DOE URL and verify HTTP 200")
})
...
test_that("Can download Utah enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Utah enrollment file", {
  skip_if_offline()
...
test_that("utschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- utschooldata::get_available_years()
  ...
  tryCatch({
    raw <- utschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- utschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- utschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- utschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- utschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- utschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- utschooldata:::get_cache_path(2024, "enrollment")
